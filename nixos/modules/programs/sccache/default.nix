...
in {
  options.programs.sccache = {
    # host configuration
    enable = lib.mkEnableOption (lib.mdDoc "Sccache");
    cache = lib.mkOption {
      type = import ./cache_submodule.nix {inherit lib;};
    };
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.enable {
      ...
      systemd.tmpfiles.rules = with cfg.cache.disk; lib.mkIf enable ["d ${cacheDir} 0770 ${cfg.owner} ${cfg.group} -"];
